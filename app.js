...
const HEXAGRAM_SHEET_URL =
  readEnv("EXPO_PUBLIC_HEXAGRAM_SHEET_URL") || readEnv("HEXAGRAM_SHEET_URL") || null;
const HEXAGRAM_SHEET_ID =
  readEnv("EXPO_PUBLIC_HEXAGRAM_SHEET_ID") || readEnv("HEXAGRAM_SHEET_ID") || null;
const HEXAGRAM_SHEET_TAB =
  readEnv("EXPO_PUBLIC_HEXAGRAM_SHEET_TAB") || readEnv("HEXAGRAM_SHEET_TAB") || "Sheet1";

const resolveHexagramSheetUrl = () => {
  if (HEXAGRAM_SHEET_URL) return HEXAGRAM_SHEET_URL;
  if (HEXAGRAM_SHEET_ID && HEXAGRAM_SHEET_TAB)
    return `https://opensheet.elk.sh/${HEXAGRAM_SHEET_ID}/${HEXAGRAM_SHEET_TAB}`;
...
export const normalizeHexagramRow = (row) => {
  ...
  const changingLines = [
    pickField(map, "cl1", "cl_1", "changing_line_1"),
    pickField(map, "cl2", "cl_2", "changing_line_2"),
    pickField(map, "cl3", "cl_3", "changing_line_3"),
    pickField(map, "cl4", "cl_4", "changing_line_4"),
    pickField(map, "cl5", "cl_5", "changing_line_5"),
    pickField(map, "cl6", "cl_6", "changing_line_6"),
  ].map(clean);

  return {
    number: parseInt(pickField(map, "number", "no", "hexagram") || "0", 10) || null,
    name: clean(pickField(map, "name", "title")),
    nature: clean(pickField(map, "nature", "trigrams", "image")),
    essence: clean(pickField(map, "essence", "judgment", "judgement", "meaning")),
    description: clean(
      pickField(
        map,
        "description",
    ...
    imageUrl: clean(pickField(map, "image", "image_url", "image_url")) || null,
    linesBinary: (pickField(map, "lines") || "")
      .replace(/⚊/g, "1")
      .replace(/⚋/g, "0")
      .trim(),
    changingLines,
    judgment: clean(pickField(map, "judgment", "judgement", "meaning", "essence")),
    imageText: clean(
      pickField(map, "image_text", "image_text", "imagetext", "description")
    ),
    _raw: row,
  };
};
...
export async function loadHexagrams() {
  ...
  try {
    ...
    console.log("Error loading hexagrams:", err.message);
    return [];
  }
}
